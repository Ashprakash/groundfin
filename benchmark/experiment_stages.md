...
## Reviewer-Proofing Controls

These controls directly address the most likely adversarial-review objections.

### Control A: No-Leakage Splits

Oracle templates may use gold answers or FinanceBench justifications, but only as an upper bound. Main method claims must use templates generated from raw evidence without gold answers.

Report every result under one of these labels:

1. `oracle_template`: may use gold answer or gold justification.
2. `teacher_template`: generated from question and raw evidence only.
3. `rule_template`: generated by deterministic extraction from raw evidence only.
4. `student_template`: produced or consumed by the student model.

Main claims should be based on `teacher_template`, `rule_template`, or `student_template`, not `oracle_template`.

### Control B: Length-Matched Baselines

To avoid the criticism that templates work only because they are shorter, compare against:

1. raw evidence truncated to the same token budget,
2. extractive summary of similar length,
3. rationale-only compact evidence,
4. probabilistic template without probabilities,
5. full probabilistic template.

The template claim is credible only if the full template beats length-matched compact baselines.

### Control C: Rationale-Distillation Baseline

To avoid the criticism that this is just rationale distillation, compare:

1. answer-only distillation,
2. rationale-only distillation,
3. evidence-span distillation,
4. template-conditioned distillation,
5. template + abstention + calibration distillation.

### Control D: Probability Validation

Probabilities must be evaluated, not decorative. Report:

1. expected calibration error,
2. Brier score,
3. selective risk / coverage curves,
4. abstention accuracy,
5. correlation between support probability and correctness.

### Control E: Human Audit

Automatic scoring is directional only. For paper-grade results, manually audit a stratified subset across:

1. numeric tasks,
2. textual comparison tasks,
3. missing-evidence tasks,
4. counterfactual tasks,
5. model disagreements.
...
3. `length_matched_summary`: non-probabilistic compact text with a similar token budget.
4. `template_no_probabilities`: same template fields without probabilities.
5. `probabilistic_template`: full template.
6. `direct_grounded_evidence`: controlled evidence directly states the gold answer.

Expected pattern:

```text
gold_evidence < length_matched_summary <= template_no_probabilities < probabilistic_template <= direct_grounded_evidence
```
...
- If length-matched summaries match templates, our novelty is weak and the template must be improved.
...
- Oracle templates are allowed only for upper-bound and debugging experiments.
- Rule, teacher, and student templates must not use the gold answer.
- If a teacher model sees the gold answer, the resulting template must be labeled `oracle_template`, not `teacher_template`.
- The main paper claim must be evaluated on held-out examples where template generation uses only the question and raw evidence.
...
4. Length-matched extractive summary
5. Answer-only distillation
6. Rationale-only distillation
7. Evidence-span distillation
8. Template without probabilities
9. Template-conditioned distillation
10. Template + abstention + calibration distillation
11. Teacher model
...
- support-probability/correctness correlation
...
- No-leakage main result using teacher/rule/student templates from raw evidence only.
- Separate oracle-template upper-bound table.
- Length-matched compact-evidence baselines.
- Rationale-only and evidence-span distillation baselines.
